...
def chapter_feature_cards(annotations: list[ChapterAnnotation], continuity: WorkContinuity) -> list[dict[str, Any]]:
    ...
    for annotation in annotations:
        ...
        facts_by_id = {fact.fact_id: fact for fact in annotation.facts}
        ...
        cards.append({
            ...
            "mechanism_frames": [{
                "kind": mechanism.kind,
                "logic_relation": mechanism.logic_relation,
                "inference_level": mechanism.inference_level,
                "setup_status": mechanism.setup_status,
                "fact_kinds": sorted({
                    facts_by_id[item].kind for item in mechanism.fact_ids if item in facts_by_id
                }),
                "event_action_types": sorted({
                    events_by_id[item].action_type for item in mechanism.event_ids if item in events_by_id
                }),
                "participant_kinds": sorted(Counter(
                    entities_by_id[item].kind for item in mechanism.participant_ids if item in entities_by_id
                ).items()),
                "has_counterfactual_test": bool(mechanism.counterfactual_loss),
                "confidence": mechanism.confidence,
            } for mechanism in sorted(annotation.narrative_mechanisms, key=lambda item: item.order)],
            "scene_frames": [{
...
6. `mechanism_frames` 是已通过证据约束的深层叙事机制。模板的 purpose 与 beat_sequence 必须优先归纳其中反复出现的“驱动力—压力—选择—结果”、对比、信息揭示、情绪缓冲和开放设置组合，不能只统计事件数与事实种类。
...
def _templates_from_draft(
    ...
    for level in levels:
        ...
        for row in rows:
            ...
            minimum_beats = 5 if level == "macro" else 3
            # A shallow macro candidate is not a usable planning template.
            # Drop only that candidate; other evidence-backed templates from
            # the same synthesis may still be complete.
            if not purpose or len(beats) < minimum_beats or not tags:
                continue
