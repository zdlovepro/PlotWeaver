...
def generate_program_sequence(
    ...
    *,
    ...
    for ordinal, row in enumerate(rows, start=1):
        ...
        chapter_contract_path = Path(str(row.get("chapter_contract", "")))
        if blocked:
            result = {
        ...
        if not program_path.is_file():
            ...
            blocked = True
            continue
        if not chapter_contract_path.is_file():
            result = {
                "chapter_no": chapter_no,
                "chapter_id": str(row.get("chapter_id", "")),
                "status": "failed",
                "reason": f"章节合同不存在：{chapter_contract_path}",
            }
            results.append(result)
            blocked = True
            continue
        ...
        try:
            outcome = generate_from_program(
                ...
                chapter_contract_path=chapter_contract_path,
            ...
            if not bool(outcome.get("overall_passed")) or not isinstance(exit_payload, dict) or not bool(exit_payload.get("overall_passed")):
                ...
                result = {
                    ...
                    "chapter_contract": str(chapter_contract_path),
