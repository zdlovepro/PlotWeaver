...
def _spatial_kind(predicate: str) -> str:
    ...
    if any(token in normalized for token in ("离开", "出去", "走出", "下山", "逐出", "赶出")):
        return "leaves"
    if any(token in normalized for token in ("接近", "靠近", "趋近", "逼近", "目光可及", "远远望见")):
        return "approaches"
...
def enrich_spatiotemporal(document: ChapterDocument, annotation: ChapterAnnotation) -> ChapterAnnotation:
    """Return a schema-2.2 annotation with semantic frames and time/space relations.
    ...
    # Vehicles, rooms-as-items and other containers can legitimately be the
    # reference object of an in/on spatial fact even when their primary entity
    # type is ``item`` rather than ``location``.
    location_ids = {entity.entity_id for entity in annotation.entities if entity.kind in {"location", "item"}}
    ...
    # Several independently extracted atoms can share one exact source anchor
    # (most often where adjacent prompt windows overlap).  Their order is not
    # recoverable from offsets, so emit document-order links only between
    # distinct anchors.  The member with the furthest evidence endpoint carries
    # the transition to the next source position.
    anchor_events: list[object] = []
    for event in ordered_events:
        start = _first_start(event)
        if anchor_events and _first_start(anchor_events[-1]) == start:
            prior = anchor_events[-1]
            prior_end = max((span.end for span in getattr(prior, "evidence", ())), default=-1)
            event_end = max((span.end for span in event.evidence), default=-1)
            if event_end > prior_end:
                anchor_events[-1] = event
            continue
    ...
    temporal_relations = tuple(
        ...
        for index, (previous, current) in enumerate(zip(anchor_events, anchor_events[1:]), start=1)
    )
    ...
    return replace(
        annotation,
        scenes=tuple(scenes),
        schema_version="2.3" if annotation.narrative_mechanisms else "2.2",
