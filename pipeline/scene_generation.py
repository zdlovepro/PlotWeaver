...
from .contracts import ChapterProgram, NarrativeGraph, ParagraphDraft, SceneDraft, SceneProseValidation, SceneValidation
...
def _paragraph_style_budgets(program: ChapterProgram, budget: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    dialogue_candidates = sorted(
        (
            ...
            if (
                ...
                or bool(paragraph.dialogue_act)
                or any(
                    len(event_by_id[event_id].participant_ids) >= 2
    ...
    dialogue_target = float(metrics.get("dialogue_char_ratio", {}).get("target", 0) or 0)
    ...
    planned_char_count = sum(prose_target(paragraph, paragraph.paragraph_id in short_ids) for paragraph in all_paragraphs)
    dialogue_role_count = max(1, len(role_ids["对话"]))
    dialogue_minimum = max(16, round(planned_char_count * dialogue_target / dialogue_role_count * 0.75)) if role_ids["对话"] else 0
    role_instructions = {
    ...
    for paragraph in all_paragraphs:
        ...
        if "对话" in labels:
            instructions[labels.index("对话")] += f" 对白总字数不得少于{dialogue_minimum}字，且只能由当前场景参与者说出。"
        ...
        if "对话" in labels:
            result[paragraph.paragraph_id]["最低对白字数"] = dialogue_minimum
    return result
    ...
    *,
    ...
    for _ in range(max(1, contract_attempts)):
        ...
        try:
            ...
            error = str(exc)
    raise ValueError(f"模型 JSON 连续不符合场景契约：{error}")
...
def generate_from_program(
    ...
    *,
    ...
    for index, (scene, scene_payload) in enumerate(zip(program.scene_programs, scene_payloads), start=1):
        ...
        completed = _completed_scene(result_path, scene, minimum_chars, scene_paragraph_budgets, entity_names)
        ...
        if draft is None:
            ...
            for batch_no, paragraph_ids in enumerate(batch_groups, start=1):
                ...
                if batch_draft is None:
                    batch_payload = _batch_scene_payload(
                        scene_payload,
                        paragraph_ids,
                        include_entry_state=batch_no == 1,
                        include_exit_state=batch_no == len(batch_groups),
                    )
                    ...
                    batch_subgraph = {
                        "段落子图": [
                            narrative_subgraph_for_paragraph(graph, runtime_graph, program, scene, paragraphs_by_id[paragraph_id])
                    ...
                    draft_prompt = _render(folder / "prompt_templates" / "scene_draft.md", {
                        ...
                        "SCENE_PROGRAM_JSON": batch_payload,
                        "NARRATIVE_SUBGRAPH_JSON": batch_subgraph,
                    })
                    try:
    ...
    graph_patch_validation = validate_graph_patch(graph, runtime_graph, graph_patch)
