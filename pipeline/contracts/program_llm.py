...
def _event_from_llm(payload: dict[str, Any]) -> EventProgram:
    context = "必现事件"
    _require_exact_keys(payload, {"事件ID", "场景ID", "事件概述", "行动", "参与实体ID", "阻力", "决断", "前置事实ID", "必须表达的事实ID", "结果事实ID"}, context)
    return EventProgram(
...
def chapter_program_from_llm_dict(payload: dict[str, Any]) -> ChapterProgram:
    ...
    for item in payload["场景程序"]:
        ...
        scene_context = "场景程序"
        _require_exact_keys(item, {"场景ID", "章节ID", "场景序号", "场景目标", "参与实体ID", "入场状态事实ID", "必现事件", "事实表达契约", "戏剧节拍", "段落程序", "出场状态事实ID", "禁止提前事件ID"}, scene_context)
